...
impl CustomOp1 for AllReduce {
    ...
    fn cuda_fwd(
        ...
        let dst = match storage.dtype() {
            DType::BF16 => {
                let slice = storage.as_cuda_slice::<bf16>()?;
                let slice = match layout.contiguous_offsets() {
                    Some((0, l)) if l == slice.len() => slice,
                    Some(_) | None => candle::bail!("input has to be contiguous"),
                };
                let mut dst = unsafe { device.alloc::<bf16>(elem_count) }.w()?;
                self.comm
                    .all_reduce(slice, &mut dst, &ReduceOp::Sum)
                    .map_err(candle::Error::debug)?;
                candle::CudaStorage::wrap_cuda_slice(dst, device)
            }
            DType::F16 => {
                let s = storage.as_cuda_slice::<f16>()?;
                let s = match layout.contiguous_offsets() {
...
impl Llama {
    ...
    pub fn load(vb: VarBuilder, cache: &Cache, cfg: &Config, comm: Rc<Comm>) -> Result<Self> {
        ...
        let norm: RmsNorm = rms_norm(cfg.hidden_size, 1e-5, vb.pp("model.norm"))?;
