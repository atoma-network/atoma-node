...
pub struct StartChatRequest {
    ...
    pub chat_id: String,
    /// The public key of the user initiating the chat.
    pub user_pk: String,
    /// The maximum number of input tokens allowed in the chat.
    pub max_input_tokens: usize,
    /// The maximum number of messages allowed in the chat.
    pub max_messages: usize,
    /// The maximum number of output tokens allowed in the chat.
    pub max_output_tokens: usize,
    /// The model id of the LLM model that the node is currently serving
    pub model_id: String,
    /// The output destination for each of the messages generated
    /// through the chat session
    pub output_destination: OutputDestination,
    /// The random seed used for generating responses.
    pub random_seed: u64,
}
...
/// Represents the parameters used for generating responses in the chat model.
/// This struct encapsulates various settings that influence the behavior
/// of the model during the generation process, allowing for fine-tuning
...
pub struct GenerateParameters {
    ...
    /// more deterministic.
    pub temperature: f64,
    /// The repeat penalty applies a penalty to repeated tokens in the output,
    /// discouraging the model from generating the same token multiple times
    /// in a single response.
    pub repeat_penalty: f32,
    /// The repeat last n specifies how many of the most recent tokens should
    /// be considered when applying the repeat penalty, helping to maintain
    /// variety in the generated text.
    pub repeat_last_n: u64,
    /// The maximum number of tokens that can be generated in a single response,
    /// limiting the length of the output to ensure it remains concise and relevant.
    pub max_tokens: u64,
    /// The optional top k value restricts the sampling to the top k most likely
    /// next tokens, enhancing the quality of the generated text by focusing on
    /// the most probable options.
    pub top_k: Option<u64>,
    /// The optional top p value (nucleus sampling) considers the smallest set of
    /// tokens whose cumulative probability exceeds the specified threshold p,
    /// allowing for more flexible and contextually appropriate outputs.
...
impl TryFrom<(Value, String)> for StartChatRequest {
    ...
    fn try_from((value, user_pk): (Value, String)) -> Result<Self, Self::Error> {
        let output_destination = value["output_destination"]
            .as_array()
            .ok_or(anyhow!(
                "Request malformed, missing `output_destination` field"
        ...
        Ok(Self {
            chat_id: value["ticket_id"]
                .as_str()
                .ok_or_else(|| anyhow!("Missing `chat_id` field"))?
                .to_string()
                .replace("0x", ""),
            user_pk,
            max_input_tokens: value["max_input_tokens"]
                .as_u64()
                .ok_or_else(|| anyhow!("Missing `max_input_tokens` field"))?
                .try_into()?,
            max_messages: value["max_messages"]
                .as_u64()
                .ok_or_else(|| anyhow!("Missing `max_messages` field"))?
                .try_into()?,
            max_output_tokens: value["max_output_tokens"]
                .as_u64()
                .ok_or_else(|| anyhow!("Missing `max_output_tokens` field"))?
                .try_into()?,
            model_id: value["model_id"]
                .as_str()
                .ok_or_else(|| anyhow!("Missing `model_id` field"))?
                .to_string(),
            random_seed: value["random_seed"]
                .as_u64()
                .ok_or_else(|| anyhow!("Missing `random_seed` field"))?,
            output_destination,
        })
    }
}
